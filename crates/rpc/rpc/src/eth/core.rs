    ...
    Rpc: RpcConvert,
{
    ...
    pub fn wrap_state(&self, state: StateProviderBox) -> StateProviderBox {
        match &self.state_provider_interceptor {
            Some(interceptor) => interceptor.wrap(state),
            None => state,
        }
    }

    /// Sets the state provider interceptor used to wrap returned state providers.
    pub fn set_state_provider_interceptor(
        &mut self,
        state_provider_interceptor: Option<StateProviderInterceptor>,
    ) {
        self.state_provider_interceptor = state_provider_interceptor;
    }
}
